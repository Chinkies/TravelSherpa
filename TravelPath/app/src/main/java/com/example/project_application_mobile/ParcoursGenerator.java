...
import java.util.List;
    ...
    public class ParcoursGenerator {
        ...
        public List<Lieu> filtrerLieu(PreferenceUtilisateur prefs, List<Lieu> toutLeslieux, boolean meteoActuelleMauvaise) {
            ...
            for (Lieu lieu : toutLeslieux) {
                ...
                if (lieu.isEstEnExterieur()) {
                  ...
                  if (meteo.contains("Froid") || meteo.contains("Humidité") ||
        ...
        public List<Parcours> optionGenerator(PreferenceUtilisateur prefs, List<Lieu> lieuxFiltres) {
            ...
            optionsProposees.addAll(genererVariations("Économique", lieuxEco, prefs.getBudgetMax(), nbJours, 3));
            ...
            optionsProposees.addAll(genererVariations("Confort", lieuxConfort, prefs.getBudgetMax(), nbJours, 3));

            List<Lieu> lieuxEquilibre = new ArrayList<>(lieuxFiltres);
            Collections.shuffle(lieuxEquilibre);
            optionsProposees.addAll(genererVariations("Équilibré", lieuxEquilibre, prefs.getBudgetMax(), nbJours, 3));

            return optionsProposees;
        }

        private List<Parcours> genererVariations(String type, List<Lieu> lieuxTries, double budgetMax, int nbJours, int nbVariations) {
            List<Parcours> resultats = new ArrayList<>();
            int maxStarts = Math.min(nbVariations, lieuxTries.size());

            for (int i = 0; i < maxStarts; i++) {
                List<Lieu> lieuxDisponibles = new ArrayList<>(lieuxTries);

                Lieu lieuDepart = lieuxDisponibles.remove(i);
                lieuxDisponibles.add(0, lieuDepart);

                String nomDynamique = type + " autour de " + lieuDepart.getName();
